...
public class AddressBook {
    ArrayList<ContactDetail> contactBook = new ArrayList<ContactDetail>();
    public HashMap<String, ArrayList<ContactDetail>> personsByCity = new HashMap<String, ArrayList<ContactDetail>>();
    public HashMap<String, ArrayList<ContactDetail>> personsByState = new HashMap<String, ArrayList<ContactDetail>>();
    ...
    private static ContactDetail details() {
        Scanner sc = new Scanner(System.in);
        ContactDetail person1 = new ContactDetail();
    ...
    public void searchByCity(String city, String firstName) {
        Predicate<ContactDetail> searchPerson = (contact -> contact.getCity().equals(city) && contact.getFirstName().equals(firstName));
    ...
    public void searchByState(String state, String firstName) {
        Predicate<ContactDetail> searchPerson = (contact -> contact.getState().equals(state) && contact.getFirstName().equals(firstName));
        contactBook.stream().filter(searchPerson).forEach(person -> output(person));
    }

    public void personsInCity(String city) {
        ArrayList<ContactDetail> list = personsByCity.get(city);
        list.stream().forEach(person -> output(person));
    }

    public void personsInState(String State) {
        ArrayList<ContactDetail> list = personsByState.get(State);
        list.stream().forEach(person -> output(person));
    }
    ...
    private static void output(ContactDetail person) {
    ...
    public void writeDataToCSV() throws IOException, CsvRequiredFieldEmptyException, CsvDataTypeMismatchException {
        ...
        try (Writer writer = Files.newBufferedWriter(Paths.get(fileName));) {
            ...
            StatefulBeanToCsvBuilder<ContactDetail> builder = new StatefulBeanToCsvBuilder<>(writer);
            StatefulBeanToCsv<ContactDetail> beanWriter = builder.build();
            ArrayList<ContactDetail> listOfContacts = contactBook.stream().collect(Collectors.toCollection(ArrayList::new));
